...
function _smoothTick() {
  ...
  // У верхней границы 1 явно доводим до 1, иначе последние подтемы (reachedAt === 1) не загораются
...
function SubtopicFuseEdge({
  ...
  // Сглаженный прогресс — порог reached срабатывает плавно. Эпсилон нужен, чтобы последние подтемы (reachedAt === 1) загорались при скролле в самый низ: lerp может не довести gp ровно до 1, плюс возможны расхождения viewport/контейнера.
  const gp = useSmoothProgress();
